...
# Normalize DeviceHub strings generated by Python patches. Literal newlines inside
# a quoted Kotlin string cause compile errors; convert them to escaped newlines.
p = Path('mobile/src/main/java/com/jarvis/mobile/DeviceHubActivity.kt')
s = p.read_text()

# Legacy long-token Remote text.
broken_hub = '''            Toast.makeText(this, "Remote activado. Usa una VPN privada para acceder desde fuera de tu Wi‑Fi.
IP: ${localIp()}:${PhoneBridgeService.PORT}
Token: $token", Toast.LENGTH_LONG).show()'''
fixed_hub = '''            Toast.makeText(this, "Remote activado. Usa una VPN privada para acceder desde fuera de tu Wi‑Fi.\\nIP: ${localIp()}:${PhoneBridgeService.PORT}\\nToken: $token", Toast.LENGTH_LONG).show()'''
if broken_hub in s:
    s = s.replace(broken_hub, fixed_hub)

# Current four-digit PIN Remote text generated by patch_remote_control.py.
broken_pin_activate = '''            Toast.makeText(this, "Remote activado.
IP del móvil: ${localIp()}
PIN para la TV: $pin

En la misma Wi‑Fi no necesitas VPN.", Toast.LENGTH_LONG).show()'''
fixed_pin_activate = '''            Toast.makeText(this, "Remote activado.\\nIP del móvil: ${localIp()}\\nPIN para la TV: $pin\\n\\nEn la misma Wi‑Fi no necesitas VPN.", Toast.LENGTH_LONG).show()'''
if broken_pin_activate in s:
    s = s.replace(broken_pin_activate, fixed_pin_activate)

broken_pin_show = '''            Toast.makeText(this, "IP del móvil: ${localIp()}
PIN de emparejamiento: $pin", Toast.LENGTH_LONG).show()'''
fixed_pin_show = '''            Toast.makeText(this, "IP del móvil: ${localIp()}\\nPIN de emparejamiento: $pin", Toast.LENGTH_LONG).show()'''
if broken_pin_show in s:
    s = s.replace(broken_pin_show, fixed_pin_show)

# Fail before Gradle with a clear message if any known multiline Toast survived.
if 'Toast.makeText(this, "Remote activado.\nIP del móvil:' in s:
    raise SystemExit('Generated DeviceHub Kotlin still contains multiline Remote PIN text')
if 'Toast.makeText(this, "IP del móvil: ${localIp()}\nPIN de emparejamiento:' in s:
