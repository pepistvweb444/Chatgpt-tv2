...
s=s.replace('''        cancelled = false
        val resolved = resolveTask(task)''','''        cancelled = false
        recentAgentActions.clear()
        lastUiSignature = ""
        unchangedUiCount = 0
        val resolved = resolveTask(task)''',1)
...
# Track whether UI actually changed before asking planner for another step.
needle='''            val pkg = prefs.getString("foreground_package", "").orEmpty()
            val action = plan(task, ui, pkg, step)'''
repl='''            val pkg = prefs.getString("foreground_package", "").orEmpty()
            val sig = uiSignature(ui, pkg)
...
if needle not in s: raise SystemExit('planner loop anchor not found')
