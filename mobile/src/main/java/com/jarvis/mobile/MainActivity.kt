...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        conversationId=prefs.getString("currentConversation",null)?:newConversation(); loadConversation(); refreshDrawerRecents(); restoreSelectedTools()
        warmLocation()
        ...
        findViewById<View>(R.id.homeAutomation).setOnClickListener{activeWidgetKind="home";showWidget("Domótica","⌂  Casa conectada\n\nConsultando luces, clima, escenas y energía…");sendAutomationPrompt("Muéstrame el estado de mi domótica y los dispositivos de casa")}
        findViewById<View>(R.id.dayWidget).setOnClickListener{activeWidgetKind="day";showWidget("Resumen del día","✦  Preparando tu resumen\n\nAgenda · recordatorios · asuntos importantes");sendAutomationPrompt("Dame mi resumen del día con agenda, recordatorios y asuntos importantes")}
        findViewById<View>(R.id.newsWidget).setOnClickListener{activeWidgetKind="news";showWidget("Noticias","▣  Actualizando noticias importantes…");sendAutomationPrompt("Muéstrame las noticias más importantes de hoy con imágenes o vídeos cuando existan")}
    ...
    private fun bestLastKnownLocation():Location?{
        ...
        return providers.mapNotNull{p->runCatching{lm.getLastKnownLocation(p)}.getOrNull()}.maxByOrNull{it.time}
    }

    private fun openWeatherForCurrentLocation(){
        activeWidgetKind="weather"
        if(!hasLocationPermission()){
            showWidget("Tiempo · tu ubicación","☁  Necesito permiso de ubicación para mostrar el tiempo donde estás ahora.")
        ...
        if(loc==null){showWidget("Tiempo · tu ubicación","☁  Buscando tu ubicación actual…\n\nActiva la ubicación del teléfono si está desactivada.");Toast.makeText(this,"No se pudo obtener todavía la ubicación",Toast.LENGTH_SHORT).show();return}
        lastLocation=loc
        showWidget("Tiempo · tu ubicación actual","☁  Consultando el tiempo donde estás ahora…")
        sendAutomationPrompt("Dime el tiempo actual y la previsión de hoy exactamente para mi ubicación actual: latitud ${loc.latitude}, longitud ${loc.longitude}. No me preguntes ciudad. Usa estas coordenadas.")
    }
    ...
    private fun updateWidgetFromReply(question:String,reply:String){val lower=question.lowercase();val kind=activeWidgetKind?:when{lower.contains("tiempo")||lower.contains("previsión")->"weather";lower.contains("domótica")||lower.contains("casa")->"home";lower.contains("noticias")->"news";lower.contains("resumen")||lower.contains("agenda")->"day";else->null}?:return;val clean=reply.replace(Regex("https?://\\S+"),"").replace(Regex("[*_`#>|]+")," ").trim();when(kind){"weather"->showWidget("Tiempo · tu ubicación actual","☁  ${clean.take(900)}");"home"->showWidget("Domótica · estado de casa","⌂  ${clean.take(900)}");"news"->showWidget("Noticias · ahora","▣  ${clean.take(900)}");"day"->showWidget("Resumen del día","✦  ${clean.take(900)}")};activeWidgetKind=kind}

    private fun startNewChat(){conversationId=newConversation();transcript.text="";activeWidgetKind=null;widgetHost.visibility=View.GONE;welcomePanel.visibility=View.VISIBLE;recentChats.text="¿En qué puedo ayudarte hoy?";refreshDrawerRecents()}
