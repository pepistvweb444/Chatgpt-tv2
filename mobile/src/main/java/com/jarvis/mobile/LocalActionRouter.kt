...
class LocalActionRouter(private val activity: Activity) {
    ...
    fun handle(raw: String): Result {
        ...
        when {
            ...
            lower.contains("whatsapp") && listOf("lee","leeme","mensajes","escrito","que dice").any { lower.contains(it) } -> return readNotificationMessages("whatsapp", 12)
            lower.contains("rcs") && listOf("lee","leeme","mensajes").any { lower.contains(it) } -> return readNotificationMessages("messages", 12)
            lower.contains("ultimos mensajes") || lower.contains("ultimos sms") || lower == "lee mis mensajes" || lower == "leeme mis mensajes" || lower == "lee los mensajes" || lower == "mis mensajes" || lower == "mensajes" -> return readRecentSms(10)
    ...
    private fun phoneAccessStatus(): Result {
        ...
        return Result(true,"Llamadas: ${if(calls)"habilitadas" else "falta permiso"}. Contactos: ${if(contacts)"habilitados" else "falta permiso"}. Puedes decir: llama a Marta.")
    }
    ...
    private fun messageAccessStatus(): Result {
        ...
        if(!listener) runCatching { activity.startActivity(Intent("android.settings.ACTION_NOTIFICATION_LISTENER_SETTINGS")) }
        return Result(true,"SMS: ${if(read)"lectura habilitada" else "concede el permiso que aparece"}. WhatsApp/RCS: ${if(listener)"acceso habilitado" else "activa Jarvis en Acceso a notificaciones"}.")
    }
    ...
    private fun readNotificationMessages(filter:String,limit:Int):Result{
        ...
        if(out.isEmpty()&&!connected) runCatching{activity.startActivity(Intent("android.settings.ACTION_NOTIFICATION_LISTENER_SETTINGS"))}
        return Result(true,if(out.isEmpty())"Todavía no tengo mensajes capturados de $filter. ${if(!connected)"Activa Jarvis en Acceso a notificaciones; te he abierto esa pantalla." else "El acceso está activo; espera a que llegue una notificación nueva."}" else "Mensajes recientes:\n\n"+out.joinToString("\n\n"))
    }
    ...
    private fun callNumber(number:String):Result{if(ContextCompat.checkSelfPermission(activity,Manifest.permission.CALL_PHONE)!=PackageManager.PERMISSION_GRANTED){ActivityCompat.requestPermissions(activity,arrayOf(Manifest.permission.CALL_PHONE),70);return Result(true,"Concede permiso de Teléfono y repite la orden.")};val clean=number.filter{it.isDigit()||it=='+'};return try{activity.startActivity(Intent(Intent.ACTION_CALL,Uri.parse("tel:${Uri.encode(clean)}")));Result(true,"Llamando a $clean.")}catch(e:Exception){activity.startActivity(Intent(Intent.ACTION_DIAL,Uri.parse("tel:${Uri.encode(clean)}")));Result(true,"He abierto el marcador con $clean.")}}
    private fun callContact(name:String):Result{
        if(name.isBlank())return Result(true,"Dime a quién quieres llamar."); val nc=ContextCompat.checkSelfPermission(activity,Manifest.permission.READ_CONTACTS)!=PackageManager.PERMISSION_GRANTED;val np=ContextCompat.checkSelfPermission(activity,Manifest.permission.CALL_PHONE)!=PackageManager.PERMISSION_GRANTED
        if(nc||np){ActivityCompat.requestPermissions(activity,buildList{if(nc)add(Manifest.permission.READ_CONTACTS);if(np)add(Manifest.permission.CALL_PHONE)}.toTypedArray(),70);return Result(true,"Concede Contactos y Teléfono y repite la orden.")}
