...
object JarvisSync {
    ...
    fun force(context: Context) {
        val app = context.applicationContext
        Thread { runCatching { syncOnce(app) } }.start()
    }

    private fun syncOnce(context: Context) {
        val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val syncKey = prefs.getString("sync_key", "").orEmpty()
        if (syncKey.length < 16) return
        ...
        val get = (URL("$BACKEND/api/sync").openConnection() as HttpURLConnection).apply {
            ...
            setRequestProperty("X-Jarvis-Sync-Key", syncKey)
            setRequestProperty("Accept", "application/json")
        }
        val getBody = (if (get.responseCode in 200..299) get.inputStream else get.errorStream)?.bufferedReader()?.use { it.readText() }.orEmpty()
        if (get.responseCode !in 200..299) return
        val remote = JSONObject(getBody)
        val remoteUpdated = remote.optLong("updatedAt", 0L)

        if (remoteUpdated > localUpdated) {
            val state = remote.optJSONObject("state") ?: JSONObject()
            applyState(prefs, state)
            prefs.edit().putLong("sync_updated_at", remoteUpdated).putLong("sync_last_ok", System.currentTimeMillis()).apply()
            return
        }

        val state = snapshot(prefs)
        ...
        val payload = JSONObject().put("updatedAt", if (localUpdated > 0) localUpdated else System.currentTimeMillis()).put("state", state).toString()
    ...
    private fun snapshot(prefs: SharedPreferences): JSONObject {
        ...
        for ((k, v) in prefs.all) {
            ...
            when (v) {
                ...
                is Set<*> -> out.put(k, JSONArray(v.toList()))
            }
        }
        return out
    }
    ...
    private fun shouldSync(k: String): Boolean =
        k == "chatIndex" || k == "currentConversation" || k == "mcps" || k == "voice" ||
        k == "assistantName" || k == "wakeWord" || k == "notification_feed" || k == "call_feed" ||
        k == "last_incoming_call" || k == "notification_listener_connected" || k.startsWith("chat_") ||
        k.startsWith("response_") || k.startsWith("phone_") || k.startsWith("reminder_")
}
