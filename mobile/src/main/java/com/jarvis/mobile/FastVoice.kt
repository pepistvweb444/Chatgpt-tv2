...
object FastVoice {
    ...
    private fun speechText(raw:String):String{
        ...
        s=s.replace(Regex("[^]*"),"")
        s=s.replace(Regex("[*_`#>|~^=+\\\\/{}\\[\\]<>]+")," ")
        s=s.replace("&"," y ").replace("%"," por ciento ").replace("€"," euros ").replace("$"," ").replace("@"," ")
        // Noiz/OpenVoice clone occasionally mishandles ñ. This phonetic spelling keeps Spanish pronunciation natural.
        s=s.replace("ñ","ni").replace("Ñ","Ni")
        // Keep letters, numbers and normal spoken punctuation only; discard decorative/technical symbols and emoji.
        s=s.replace(Regex("[^\\p{L}\\p{N} áéíóúüÁÉÍÓÚÜ,.;:!?¿¡()-]+")," ")
        s=s.replace(Regex("(?m)^\\s*[-•]+\\s*"),"")
        return s.replace(Regex("[\\r\\n]+")," ").replace(Regex("\\s+")," ").replace(Regex("\\s+([,.;:!?])"),"$1").trim()
    }
    private fun chunk(text:String):List<String>{val out=mutableListOf<String>();var rest=text;var first=true;while(rest.isNotBlank()){val max=if(first)48 else 92;if(rest.length<=max){out+=rest;break};val w=rest.take(max+1);var cut=listOf(w.lastIndexOf('.'),w.lastIndexOf('?'),w.lastIndexOf('!'),w.lastIndexOf(','),w.lastIndexOf(' ')).maxOrNull()?:max;if(cut<if(first)22 else 38)cut=max;val take=(cut+if(w.getOrNull(cut) in listOf('.','?','!',','))1 else 0).coerceAtMost(rest.length);out+=rest.take(take).trim();rest=rest.drop(take).trim();first=false};return out.filter{it.isNotBlank()}}
    private fun downloadClone(activity:Activity,text:String,index:Int):File{val c=(URL("$BACKEND/api/speech").openConnection() as HttpURLConnection).apply{requestMethod="POST";doOutput=true;connectTimeout=1800;readTimeout=8500;setRequestProperty("Content-Type","application/json; charset=utf-8");setRequestProperty("Accept","audio/*");setRequestProperty("Connection","keep-alive")};c.outputStream.use{it.write(JSONObject().put("text",text).put("voice","mi_voz").put("provider","race-clone").put("speed",1.18).toString().toByteArray())};if(c.responseCode !in 200..299)throw IllegalStateException("TTS ${c.responseCode}");return File(activity.cacheDir,"jarvis-clone-${System.currentTimeMillis()}-$index.mp3").also{f->c.inputStream.use{input->f.outputStream().use{input.copyTo(it)}}}}
