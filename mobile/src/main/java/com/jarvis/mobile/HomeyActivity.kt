...
class HomeyActivity : Activity() {
    ...
    private fun refresh() {
        ...
        Thread {
            try {
                ...
                prefs.edit().putString("homey_devices_json",devices.toString()).apply { if(renewed.isNotBlank()) putString("homey_session",renewed) }.apply()
    ...
    private fun render(devices: JSONArray) {
        ...
        for(i in 0 until devices.length()) {
            val d=devices.optJSONObject(i)?:continue; val name=d.optString("name").ifBlank{"Dispositivo"}; val type=d.optString("type"); val state=d.optJSONObject("state")?:JSONObject()
            val on=if(state.has("onoff")&&!state.isNull("onoff")) state.optBoolean("onoff") else null
    ...
    private fun setCapability(deviceId:String,capability:String,value:Any,name:String) {
        ...
        Thread {
            try {
                ...
                val o=JSONObject(raw); val arr=o.optJSONArray("devices")?:JSONArray(); val renewed=o.optString("session"); prefs.edit().putString("homey_devices_json",arr.toString()).apply{if(renewed.isNotBlank())putString("homey_session",renewed)}.apply()
