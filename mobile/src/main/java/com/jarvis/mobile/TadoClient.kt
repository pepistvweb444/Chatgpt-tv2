...
object TadoClient {
    ...
    fun setPower(context: Context, zone: Zone, on: Boolean, target: Double? = null) {
        val token = validToken(context)
        val state = runCatching { JSONObject(get("$API/homes/${zone.homeId}/zones/${zone.id}/state", token)) }.getOrElse { JSONObject() }
        ...
        // Tado AC zones often require the currently supported mode/fan setting to be sent
        // together with power/temperature. Reuse the live setting and only change what Jarvis
        // was asked to change, instead of constructing an incomplete setting that yields 422.
        val setting = JSONObject(current.toString())
        setting.put("type", type)
        setting.put("power", if (on) "ON" else "OFF")
        ...
        if (on && target != null && type != "HOT_WATER") {
            ...
            val tempCaps = limits?.optJSONObject("temperatures")
                ?: limits?.optJSONObject("modes")?.optJSONObject(mode)?.optJSONObject("temperatures")
            val min = tempCaps?.optDouble("celsius", Double.NaN)?.takeUnless { it.isNaN() }
            val minObj = tempCaps?.optJSONObject("celsius")
            val minC = minObj?.optDouble("min", Double.NaN)?.takeUnless { it.isNaN() } ?: min ?: 5.0
            val maxC = minObj?.optDouble("max", Double.NaN)?.takeUnless { it.isNaN() } ?: 30.0
            setting.put("temperature", JSONObject().put("celsius", target.coerceIn(minC, maxC)))
        }
    ...
    private fun request(method: String, url: String, token: String, body: String?): String {
        ...
        if (code !in 200..299) {
            ...
            throw IllegalStateException("Tado HTTP $code ${text.take(240)}$rateInfo")
        }
        return text
    }
