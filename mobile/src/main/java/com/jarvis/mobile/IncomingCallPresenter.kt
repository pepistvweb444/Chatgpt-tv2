...
object IncomingCallPresenter {
    ...
    fun show(context: Context, call: org.json.JSONObject) {
        ...
        val open = PendingIntent.getActivity(context, 5107, activityIntent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        fun action(label: String, value: String, request: Int): NotificationCompat.Action {
            val pi = PendingIntent.getBroadcast(context, request, Intent(context, CallActionReceiver::class.java).putExtra("call_action", value), PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        ...
        val publicLabel = call.optString("publicLabel")
        val title = if (known) call.optString("name") else publicLabel.ifBlank { call.optString("name") }.ifBlank { call.optString("number").ifBlank { call.optString("app").ifBlank { "Llamada entrante" } } }
        val detail = when (classification) {
            "checking" -> "Comprobando número en Internet…"
        ...
        val builder = NotificationCompat.Builder(context, CHANNEL)
            ...
            .setStyle(NotificationCompat.BigTextStyle().bigText(buildString {
                append(detail)
                call.optString("number").takeIf { it.isNotBlank() && it != title }?.let { append("\n").append(it) }
                ...
                call.optString("spamSources").takeIf { it.isNotBlank() }?.let { append("\nFuentes spam: ").append(it) }
            }))
            .setCategory(NotificationCompat.CATEGORY_CALL)
            .setPriority(NotificationCompat.PRIORITY_MAX)
            .setVisibility(NotificationCompat.VISIBILITY_PUBLIC)
            .setOngoing(true).setAutoCancel(false).setContentIntent(open).setFullScreenIntent(open, true)
        ...
        if (photo.isNotBlank()) runCatching { val bytes = Base64.decode(photo, Base64.DEFAULT); builder.setLargeIcon(BitmapFactory.decodeByteArray(bytes, 0, bytes.size)) }
        nm.notify(NOTIFICATION_ID, builder.build())
        runCatching { context.startActivity(activityIntent) }
    }

    fun dismiss(context: Context) { context.getSystemService(NotificationManager::class.java).cancel(NOTIFICATION_ID) }
}
