...
# Accessibility: add IME submit so product search can be committed even when Makro
# exposes no visible Buscar button.
p=Path('mobile/src/main/java/com/jarvis/mobile/JarvisAccessibilityService.kt')
s=p.read_text()
if 'ACTION_IME_ENTER' not in s:
    ...
    method='''    private fun imeEnter() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 30) node?.performAction(AccessibilityNodeInfo.ACTION_IME_ENTER)
        persistUiSnapshot()
    }
...
helpers=r'''    private fun makroProductFromTask(task: String): String {
    ...
    private fun makroDeterministicAction(task: String, ui: JSONArray, pkg: String): JSONObject? {
...
p.write_text(s)
print('Makro deterministic search -> suggestion/result -> add-to-cart flow applied')
