...
export default async function handler(req,res){
  try{
    ...
    const body=new URLSearchParams({grant_type:'authorization_code',authorization_code:code,redirect_uri:redirect});
    const r=await fetch('https://api.athom.com/oauth2/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','Authorization':'Basic '+Buffer.from(`${clientId}:${clientSecret}`).toString('base64')},body});
    const token=await r.json().catch(()=>({})); if(!r.ok) return res.status(502).send(token?.error_description||token?.error||'Homey token error');
